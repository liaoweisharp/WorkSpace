    ...
    try:
        for item in json["data"]["lists"]:
            try:
                ...
                obj["id"]="SEED_Tab_Phone_Songs_Keys4"
...
def db(dbDAL,objList):
    ...
    dbDAL.insert(tableName="Tab_Phone_Songs_Keys4", list=objList,sequenceList=["id"])

def step1():
    resultValue = []
    dbDAL = DataBase.DatabaseDAL_Oracle.DatabaseDAL()
    sql = "select  song_Name,singer,id  from Tab_Phone_Songs4 a where rownum=1 and lock1 is null and song_Name is not null order by id"
    ...
    while len(list)==1:
        ...
        dbDAL.update(tableName="Tab_Phone_Songs4", list=updateList, whereTuple={"id", })

        _list = getOneSong(obj)
        logger.info("obj={0}".format(obj))
        logger.info("len={0}".format(len(_list)))
...
def step2():
    try:
        ...
        while True:
            resultValue = []
            dbDAL = DataBase.DatabaseDAL_Oracle.DatabaseDAL()
            sql = "select id  from Tab_Phone_Songs4 a where rownum=1 and lock2 is null and song_Name is not null  order by id"
            list = dbDAL.query(sql)
            if len(list) == 0:
                break
            if len(list) == 1:
                ...
                sql = "select id,temp1,song_Id  from Tab_Phone_Songs_Keys4 where  song_Id={0} and temp1 is not null and keys is null".format(id)
                list = dbDAL.query(sql)
                num=0
                while num < len(list):
