    ...
    try:
        for item in json["data"]["song"]["list"]:
            try:
                ...
                obj["id"] = "SEED_Tab_Phone_Songs_Keys{0}".format(app_Id)
                obj["song_Id"]=id
                obj["app_Id"]=app_Id
...
def db(dbDAL,objList):
    ...
    dbDAL.insert(tableName="Tab_Phone_Songs_Keys1", list=objList,sequenceList=["id"])

def main():
    resultValue=[]
    dbDAL = DataBase.DatabaseDAL_Oracle.DatabaseDAL()
    sql = '''select song_Name,singer,id  from Tab_Phone_Songs1 a 
where rownum=1 and lock1 is null and song_Name is not null and id>(select case when max(song_Id) is null then 0 else max(song_Id) end  from Tab_Phone_Songs_Keys1 b where  b.app_id={0})  order by id'''.format(app_Id)
    ...
    while len(list)==1:
        ...
        dbDAL.update(tableName="Tab_Phone_Songs1", list=updateList, whereTuple={"id", })
