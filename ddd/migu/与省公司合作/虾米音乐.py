...
def getJsonData(pageObj,bsObj):
    ...
    try:
        ...
        if nodeTrs is None or len(nodeTrs)==0:
            return returnValue
        for tr in nodeTrs:
            obj={"song_Id":id,"app_Id":app_Id,"load_Date":time.strftime('%Y%m%d',time.localtime(time.time()))}
...
def main():
    ...
    sql = "select top 1 song_Name,singer,id  from Tab_Phone_Songs a where app_3 is null and  song_Name is not null and id>(select max(song_Id) from Tab_Phone_Songs_Keys b where  b.app_id={0})  order by id".format(app_Id)
