...
def getJsonData(pageObj,bsObj):
    ...
    try:
        ...
        for li in liNodes:
            try:
                obj={"song_Id":id,"app_Id":app_Id,"song_Name":"","singer_Name":"","des":"","load_Date":""}
...
def db(dbDAL,objList):
    ...
    dbDAL.insert(tableName="Tab_Phone_Songs_Keys5", list=objList)

def step1():
    resultValue = []
    dbDAL = DataBase.DatabaseDAL.DatabaseDAL(database="MiGu")
    #sql="select top 1 song_Name,singer,id  from Tab_Phone_Songs a where app_5 is null and song_Name is not null and id>(select max(song_Id) from Tab_Phone_Songs_Keys5 b where  b.app_id={0})  order by id".format(app_Id)
    # 这里要屏蔽，启用上面的
    sql = "select top 1 song_Name,singer,id  from Tab_Phone_Songs a where app_5 is null and song_Name is not null and id not in (select song_Id from Tab_Phone_Songs_Keys5 b where  b.app_id={0})  order by id".format(
        app_Id)
    ...
    try:
        while True:
            try:
                resultValue = []
                dbDAL = DataBase.DatabaseDAL.DatabaseDAL(database="MiGu")
                #sql = "select top 1 id  from Tab_Phone_Songs a where app_5_1 is null and song_Name is not null  order by id"
                #这里要屏蔽，启用上面的
                sql = "select top 1 id  from Tab_Phone_Songs a where app_5_1 is null and song_Name is not null and app_5='2' order by id"
                ...
                if len(list) == 1:
                    (id,) = list[0]
                    updateList = []
                    _obj = {"id": id, "app_5_1": "1"}
                    updateList.append(_obj)
                    dbDAL.update(tableName="Tab_Phone_Songs", list=updateList, whereTuple={"id", })

                    #开始遍历 表：Tab_Phone_Songs_Keys5
                    sql = "select id,temp1,song_Id  from Tab_Phone_Songs_Keys5 where song_Id={1} and temp1 is not null and keys is null and app_id={0}".format(app_Id,id)
                    list = dbDAL.query(sql)
                    num=0
                    while num < len(list):
